...
Source="/home/johan/diverse/ASP2_katalog"
...
if [ "$DevSource" = "$Try" ]; then
    /usr/bin/rsync -lrtvz --delete --exclude="$Exclude1" --exclude="$Exclude2" $Source $NgNas
    Nas_Success=1; 
else
...
if [ -d "$Lacie_1000" ]; then
    /usr/bin/rsync -lrtvz  --delete --exclude="$Exclude1" --exclude="$Exclude2" $Source $Lacie_1000
...
echo "Backup av ASP-katalogerna kördes den" `date` "till NAS ($Nas_Success), Lacie1000 ($Lacie_1000_Success)" >> ~/backup_dates_types.txt
