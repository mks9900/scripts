...
Source1=/run/media/johan/"NIKON D3S"/DCIM/
Source2="101NCD3S"
Source="$Source1""$Source2"
Target=$HOME/Pictures/Fotografier/import-d3s-jpg
...
# Felhantering för *.JPG finns nedan:
	...
	then 
		...
		FileTime="`exiv2 "$jpgFile"|grep timestamp|awk '{print $5}'|sed -e 's/:/-/g'`"
...
#		LensMaker="`exiv2 -pa "$jpgFile"|grep "LensIDNumber"|awk '{print $4$5$6$7}'`"
#		LensID="`exiv2 -pa "$jpgFile"|grep "LensIDNumber"|awk '{print $7}'`"
		CameraMaker="`exiv2 "$jpgFile"|grep "Camera model"|awk '{print $4}'|sed -e 's/:/-/g'`"
...
#		FileName=$FileDate"_kl-"$FileTime"_"$LensMaker"_"$FocalLength""mm_ISO$ISOSpeed"_"$Aperture
		FileName=$FileDate"_"$FileTime"_"$CameraMaker"_"$CameraModel"_"$FocalLength"_mm_ISO_"$ISOSpeed
...
for TempDir in *; do
	...
	mv -vi "$TempDir"/ "$PhotoPath"/"$Year"/"$TempDir""_""D3s_jpg"
	echo
    else
	echo "Katalogen $PhotoPath/$Year/$TempDir finns: Kopierar..."
	cp -ai "$TempDir"/*.jpg "$PhotoPath"/"$Year"/"$TempDir""_""D3s_jpg"
	echo
    fi
done
