...
Source1=/media/SanDisk_SD_SDDR-289_31216000343-1/DCIM/
Source2="134_FUJI"
Source="$Source1""$Source2"
Target=$HOME/Pictures/Fotografier/import-xt1-jpg
...
#    echo $pwd
    ...
    if [ ! -d "$PhotoPath"/"$Year"/"$TempDir""_""X-T1_jpg" ]; then
	echo "Katalogen $PhotoPath/$Year/$TempDir finns ej: Flyttar allt."
	mv -vi "$TempDir"/ "$PhotoPath"/"$Year"/"$TempDir""_""X-T1_jpg"
	echo
    else
	echo "Katalogen $PhotoPath/$Year/$TempDir finns: Kopierar..."
	cp -ai "$TempDir"/*.jpg "$PhotoPath"/"$Year"/"$TempDir""_""X-T1_jpg"
	echo
    fi
done
