...
Source1=/media/
Source2=/DCIM/103ND700
dumbAss="NIKON\\x20D700"
Source="$Source1""$dumbAss""$Source2"
Target=$HOME/Bilder/Fotografier/import
PhotoPath=$HOME/Bilder/Fotografier
...
then 
	...
	FileTime="`exiv2 "$RawFile"|grep timestamp|awk '{print $5}'|sed -e 's/:/-/g'`"
	...
	LensMaker="`exiv2 -pa "$RawFile"|grep "LensIDNumber"|awk '{print $4$5$6$7}'`"
	LensID="`exiv2 -pa "$RawFile"|grep "LensIDNumber"|awk '{print $7}'`"
	...
	if [ ! -d "$FileDate" ]; then
	    ...
	    mv -v "$RawFile" "$FileDate"/"$FileName""__nr-"$Counter.nef
	    Counter=$[Counter + 1]
	else
	    mv -v "$RawFile" "$FileDate"/"$FileName""__nr-"$Counter.nef
	    echo "$FileDate"/"$FileName""__nr-"$Counter.nef
	    Counter=$[Counter + 1]		
	fi
